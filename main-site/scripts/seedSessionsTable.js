...
const existingRows = db.prepare(`
  SELECT movie_id, cinema_id, hall, time, date
  FROM sessions
`).all();

const existing = new Set(
  existingRows.map((r) => `${r.movie_id}|${r.cinema_id}|${r.hall}|${r.time}|${r.date}`)
);
...
const seed = db.transaction(() => {
  rows.forEach((row) => {
    const key = `${row.movie_id}|${row.cinema_id}|${row.hall_number}|${row.time}|${row.date}`;
